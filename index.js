...
// O si es una imagen local, puedes poner la ruta correcta
// const imagenAntilink = './ruta/a/tu/imagen.jpg';
...
sock.ev.on('messages.upsert', async m => {
  try {
    ...
    if (!info.message) return;  // Verifica si el mensaje está presente

    const from = info.key.remoteJid; // Remitente del mensaje
    const messageText = info.message.conversation ? info.message.conversation : ''; // El texto del mensaje
    const prefix = '.';  // El prefijo para los comandos, ajusta esto si es diferente

    // Si el mensaje es un comando y empieza con el prefijo
    if (messageText.startsWith(prefix)) {
      const command = messageText.slice(prefix.length).trim();  // Extrae el comando sin el prefijo

      if (command.toLowerCase() === 'minar') {
        console.log('¡Comando minar detectado! Ejecutando...');

        // Cargar los usuarios desde el archivo usuarios.json
        let usuarios = JSON.parse(fs.readFileSync(path, 'utf8'));

        const userId = from.split('@')[0]; // Obtener el número de teléfono del usuario (sin el dominio)
        console.log(`El usuario ${userId} ha ejecutado el comando .minar`);

        // Verificar si el usuario ya tiene datos en el archivo, si no, inicializar
        if (!usuarios[userId]) {
          usuarios[userId] = { experiencia: 0 }; // Inicializamos la experiencia
        }

        // Aumentar la experiencia en 10 (puedes cambiar este valor)
        usuarios[userId].experiencia += 10;
        console.log(`El usuario ${userId} ahora tiene ${usuarios[userId].experiencia} de experiencia`);

        // Guardar los cambios en el archivo usuarios.json
        fs.writeFileSync(path, JSON.stringify(usuarios, null, 2), 'utf8');
        
        // Responder al usuario con la nueva experiencia
        sock.sendMessage(from, { text: `¡Has minado! Ahora tienes ${usuarios[userId].experiencia} de experiencia.` });
      }
    }
  } catch (error) {
...
case 'antilinkon':  // Activar antilink
    if (!isGroup) {
        return enviarTexto(mess.only.group); // Verificar si es un grupo
    }
    if (!isGroupAdmins) {
        return enviarTexto(mess.only.admin); // Verificar si es admin del grupo
    }
    if (!isBotGroupAdmins) {
        return enviarTexto(mess.only.Badmin); // Verificar si el bot es admin del grupo
    }
    // Llamar la función para activar el Anti Link
    activarAntilink(from, enviarTexto, enviarImagen);
    break;
case 'antilinkoff':  // Desactivar antilink
    ...
    // Llamar la función para desactivar el Anti Link
    desactivarAntilink(from, enviarTexto, enviarImagen);
    break;
